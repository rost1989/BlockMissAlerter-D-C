# BlockMissAlerter DNC b0.4 (Carbon)
...
token = 'XXXXXXX' #telegram token obtained from @BotFather in TG
receiver_id = 0000000 ## https://api.telegram.org/bot<TOKEN>/getUpdates Enter the link. Send message to your bot. Refresh the page. Copy receiver id
...
# ------------------------------------------------------------------------------
MaxMissedBlocks = 200
MyConsAddress = polarbearConsAddress
maxErrorsAlert = 12

# MAIN
# ------------------------------------------------------------------------------
print ('BlockMissAlerter DNC b0.4 (Carbon) is now RUNING')
sound()

while True :    
    try:        
        with urllib.request.urlopen(f"http://164.132.169.19:1317/cosmos/slashing/v1beta1/signing_infos/{MyConsAddress}") as url:
            ...
            if int(GetValConsInfo['missed_blocks_counter']) > MaxMissedBlocks:
                ...
                sound()
                bot.sendMessage(receiver_id, 'YOU ARE LOOSING BLOCKS! Missed blocks:' GetValConsInfo['missed_blocks_counter'])
